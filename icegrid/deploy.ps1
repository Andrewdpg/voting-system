...
& plink -batch -pw $password -hostkey $hostkey "swarch@$registry" "mkdir -p $remote_directory"
& pscp -batch -pw $password -hostkey $hostkey $registry_template "swarch@${registry}:$remote_directory/registry.cfg"
& plink -batch -pw $password -hostkey $hostkey "swarch@$registry" "killall icegridadmin; cd $remote_directory; mkdir -p data/registry; mkdir -p data/logs; touch data/logs/icegridregistry.log; nohup icegridregistry --Ice.Config=registry.cfg > icegrid.log 2>&1 &"
...
"@
    ...
    & plink -batch -pw $password -hostkey $hostkey "swarch@$worker_host" "mkdir -p $remote_directory"
    ...
    & plink -batch -pw $password -hostkey $hostkey "swarch@$worker_host" "killall icegridnode; cd $remote_directory; mkdir -p data; nohup icegridnode --Ice.Config=$node_config_file  > icegrid.log 2>&1 &"
}
...
"@
    ...
    $temp_file = [System.IO.Path]::GetTempFileName()
    Set-Content -Path $temp_file -Value $config_content
    & plink -batch -pw $password -hostkey $hostkey "swarch@$db_host" "mkdir -p $remote_directory"
    ...
    & plink -batch -pw $password -hostkey $hostkey "swarch@$db_host" "killall icegridnode; cd $remote_directory; mkdir -p data; nohup icegridnode --Ice.Config=$node_config_file  > icegrid.log 2>&1 &"
}
